    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent onEscapeKeyDown={(e: any) => e.preventDefault()} onPointerDownOutside={(e: any) => e.preventDefault()} className="overflow-hidden sm:max-w-[425px] bg-white dark:bg-foreground text-black-200" aria-describedby={undefined}>
          ...
          <div className="gap-3 flex flex-col">
              <div>
                <Label>Front</Label>
                <Textarea ref={termRef} onKeyDown={handleKeyDown} className="mt-2 max-h-[10rem] !text-base bg-white dark:bg-foreground border-[2px] border-dark-200 resize-none overflow-auto" placeholder="What is the powerhouse of the cell?" value={term} onChange={(e) => setTerm(e.target.value)}/>
              </div>
              <div>
                <Label>Back</Label>
                <Textarea onKeyDown={handleKeyDown} className="mt-2 max-h-[10rem] resize-none overflow-auto !text-base bg-white dark:bg-foreground border-[2px] border-dark-200" placeholder="Mitochondria" value={definition} onChange={(e) => setDefinition(e.target.value)}/>
              </div>
